              ...
              <div className="max-w-3xl mx-auto space-y-6">
                ...
                <div className="bg-zinc-900 rounded-lg p-6 border border-zinc-800">
                  ...
                  <p className="text-zinc-400 mb-4">
                    Activa el reconocimiento facial para obtener recomendaciones personalizadas basadas en tus emociones durante la visualización.
                  </p>

                  <div className="flex items-center justify-between bg-zinc-800 rounded-lg p-4 mb-3">
                    <div className="flex-1">
                      <p className="font-semibold mb-1">Estado del Análisis Emocional</p>
                      <p className="text-sm text-zinc-400">
                        {userProfile.emotionAnalysisEnabled
                          ? 'Análisis emocional activado'
                          : 'Análisis emocional desactivado'}
                      </p>
                    </div>
                    <button
                      onClick={() => {
                        if (!userProfile.emotionAnalysisEnabled) {
                          setShowEmotionConsent(true);
                        } else {
                          setUserProfile({ ...userProfile, emotionAnalysisEnabled: false });
                          setEmotionAnalysisActive(false);
                  ...
                  {emotionalSessions.length > 0 && (
                    <div className="bg-zinc-800 rounded-lg p-4">
                      <p className="font-semibold mb-2">Historial de Sesiones</p>
                      <div className="space-y-2">
                        {emotionalSessions.slice(-3).reverse().map((session, index) => (
                          <div key={index} className="flex items-center justify-between text-sm">
                            <span className="text-zinc-300">{session.movieTitle}</span>
